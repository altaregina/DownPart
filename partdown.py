...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source", type=str, default="https://getsamplefiles.com/download/txt/sample-1.txt", help="link source file"
    )
    parser.add_argument(
        "--numpart", type=int, default=4, help="Number of paritition"
    )
    parser.add_argument(
        "--name", type=str, default="sample-1.txt", help="name of downloaded file"
    )
    opt = parser.parse_args()

    url_to_file = opt.source
    if opt.name == "https://getsamplefiles.com/download/txt/sample-1.txt": 
        parts = (opt.source).split("4")
        file_name = parts[-1]
    else:
        file_name = opt.name
